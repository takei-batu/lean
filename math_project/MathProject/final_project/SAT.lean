...
lemma subst_monotone (cnf : CNF) (l : Literal) : num_var_set (substCNF cnf l) ⊆ num_var_set cnf := by
  intro c h
...
def UnitLiteral (cnf : CNF) : Option Literal :=
  let UnitClauses := cnf.filter (fun c => c.card == 1)
  let UnitVar := num_var_set UnitClauses
  match UnitVar.min with
  | some i => if {↑i} ∈ UnitClauses then some ↑i else if {-↑i} ∈ UnitClauses then some (-↑i) else none
  | none => none

lemma UnitLiteral_notin_subst_cnf (cnf : CNF) (l : Literal) : l.natAbs ∉ num_var_set (substCNF cnf l) := by
  intro h
  simp only [num_var_set, Finset.mem_biUnion, Finset.mem_image] at h
  obtain ⟨w₁, hw₁, w₂, hw₂, hw₃⟩ := h
  simp only [substCNF, Finset.mem_biUnion] at hw₁
...
lemma UnitLiteral_in_cnf {cnf : CNF} {l : Literal} (h : UnitLiteral cnf = some l) : l.natAbs ∈ num_var_set cnf := by
  ...
  next _ i _ =>
    ...
    next g =>
      ...
      simp only [Finset.mem_filter] at g
      use {↑i}
      constructor
      · exact g.left
      · use ↑i
...
def ModeLiteral (cnf : CNF) : Option Literal :=
  let SortVar := (num_var_set cnf).sort (· ≤ ·)
  (SortVar.foldl (FindMode cnf) (0, none)).2

lemma foldl_ModeLiteral {cnf : CNF} {vars : List Var} (acc : Var × Option Literal) (invariant : ∀ l, acc.2 = some l → l.natAbs ∈ num_var_set cnf) : ∀ l, (vars.foldl (FindMode cnf) acc).2 = some l → l.natAbs ∈ num_var_set cnf := by
...
lemma ModeLiteral_in_cnf {cnf : CNF} {l : Literal} (h : ModeLiteral cnf = some l) : l.natAbs ∈ num_var_set cnf := by
...
def DPLL (cnf : CNF) (n : Var) : Option Asg :=
  if _h₁ : cnf = ∅ then some []
    ...
    match _h₃ : UnitLiteral cnf with
    | some l =>
      match DPLL (substCNF cnf l) n with
      | some A => some (l :: A)
      | none => none
    | none =>
      match _h₄ : ModeLiteral cnf with
      | none => none
      | some l =>
        match DPLL (substCNF cnf l) n with
        | some A => some (l :: A)
        | none =>
          ...
          | none => none
  -- 停止性の証明
  termination_by (num_var_set cnf).card
  decreasing_by
...
def solver (path : System.FilePath) : IO Unit := do
  ...
  | some A =>
      ...
      IO.println "SAT"
      let A := A.toArray.qsort (fun a b => a.natAbs < b.natAbs) -- 見やすくするために配列に変換 & クイックソートして返す
      IO.println s!"{A}"
  | none =>
      let finish ← IO.monoMsNow
      IO.println s!"time : {finish - start} ms"
...
def writefile (path : System.FilePath) (stdout : IO Unit) : IO Unit := do
  IO.FS.withFile path IO.FS.Mode.write fun handle => do
  let stream := IO.FS.Stream.ofHandle handle
  IO.withStdout stream do stdout
...
def ex := solver "./MathProject/final_project/example1.cnf"
-- #eval ex

-- N-Queen Problem
def two_queen := solver "./MathProject/final_project/N=2.cnf"
def three_queen := solver "./MathProject/final_project/N=3.cnf"
def four_queen := solver "./MathProject/final_project/N=4.cnf"
def five_queen := solver "./MathProject/final_project/N=5.cnf"
def six_queen := solver "./MathProject/final_project/N=6.cnf"
def seven_queen := solver "./MathProject/final_project/N=7.cnf"
def eight_queen := solver "./MathProject/final_project/N=8.cnf"
def nine_queen := solver "./MathProject/final_project/N=9.cnf"
def ten_queen := solver "./MathProject/final_project/N=10.cnf"
def eleven_queen := solver "./MathProject/final_project/N=11.cnf"
def twelve_queen := solver "./MathProject/final_project/N=12.cnf"
def thirteen_queen := solver "./MathProject/final_project/N=13.cnf"
def fourteen_queen := solver "./MathProject/final_project/N=14.cnf"
def fifteen_queen := solver "./MathProject/final_project/N=15.cnf"
...
-- 8-Queen問題の結果を出力
def main : IO Unit := do
  let output := "./MathProject/final_project/N=8.txt"
  writefile output eight_queen
