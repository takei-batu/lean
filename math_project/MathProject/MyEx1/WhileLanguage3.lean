...
lemma big_small (s : Stmt) (state state' : State) : Exec s state state' →
  ...
  | SeqExec s1 s2 exec1 exec2 ih1 ih2 =>
    rename_i state1 state2 state3
    apply Relation.ReflTransGen.trans (b := (SeqStmt SkipStmt s2, state2))
    · apply small_seq
      assumption
    apply Relation.ReflTransGen.head
    · apply ExecOne.SeqExec1
    assumption
  | IfTrueExec b s1 s2 htrue exec1 ih =>
    apply Relation.ReflTransGen.head
  ...
  | WhileExecFalse b s' hfalse =>
    apply Relation.ReflTransGen.head
    · apply ExecOne.WhileExec
    apply Relation.ReflTransGen.head
    · apply ExecOne.IfFalseExec
      assumption
    · apply Relation.ReflTransGen.refl
  | WhileExecTrue b s' htrue exec1 exec2 ih1 ih2 =>
    rename_i state1 state2 state3
    apply Relation.ReflTransGen.head
    · apply ExecOne.WhileExec
    apply Relation.ReflTransGen.head
    · apply ExecOne.IfTrueExec
      · assumption
    apply Relation.ReflTransGen.trans (b := (SeqStmt SkipStmt (WhileStmt b s'), state2))
    · apply small_seq
      assumption
    apply Relation.ReflTransGen.head
    · apply ExecOne.SeqExec1
...
lemma seq_small (s1 s2 : Stmt) (state1 state2 : State) (n : Nat) :
  ...
  | succ n ih =>
    ...
    | SeqExec2 _ s1' _ step1 =>
      have f1 : RelIter ExecOne 1 (s1, state1) (s1', state') := by
        exact ⟨(s1', state'), step1, rfl⟩
      obtain ⟨l, m, state'', step2, step3, eq⟩ := ih s1' s2 state' state2 r2
      have f2 : RelIter ExecOne (l + 1) (s1, state1) (SkipStmt, state'') := by
        exact ⟨(s1', state'), step1, step2⟩
      exact ⟨l + 1, m, state'', f2, step3, by omega⟩
  ...
  | h n ih =>
    ...
    | succ n' =>
      ...
      | AssignStmt x e =>
        ...
        | zero =>
          obtain E := f
          injection f with _ eq
          rw[← eq]
          apply Exec.AssignExec
        | succ m' =>
          exfalso
          exact skip_not m' _ _ f
      | SeqStmt s1 s2 =>
        intro h
        ...
        -- have neq1 : l < n' + 1 := by omega
        have exec1 := ih l (by omega) s1 state state'' r1
        -- have neq2 : m < n' + 1 := by omega
        have exec2 := ih m (by omega) s2 state'' state' r2
        exact Exec.SeqExec s1 s2 exec1 exec2
      | IfStmt b s1 s2 =>
        intro h
        obtain ⟨_, step, f⟩ := h
        cases step with
        | IfTrueExec _ _ _ beq =>
          have exec := ih n' (by omega) s1 state state' f
          exact Exec.IfTrueExec b s1 s2 beq exec
        | IfFalseExec _ _ _ beq =>
          have exec := ih n' (by omega) s2 state state' f
          exact Exec.IfFalseExec b s1 s2 beq exec
      | WhileStmt b s =>
        intro h
        simp only [RelIter, Relation.Comp, Prod.exists] at h
        obtain ⟨s', state'', r1, r2⟩ := h
        have exec := ih n' (by omega) s' state'' state' r2
        cases r1
        cases exec with
        | IfTrueExec _ _ _ beq exec' =>
          cases exec' with
          | SeqExec _ _ exec1 exec2 =>
            exact Exec.WhileExecTrue b s beq exec1 exec2
        | IfFalseExec _ _ _ beq exec' =>
          cases exec' with
          | SkipExec =>
            exact Exec.WhileExecFalse b s beq
